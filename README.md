...
> 阶段 6：方块共建/破坏、玩家可见与移动、互殴、死亡重生、昼夜同步、聊天、**掉落物与拾取同步**、**断线自动重连**、**怪物同步（事件同步：host 生成 + 受击/死亡广播 + 位置校正）**、**红石状态同步**、**多房间（同名房间共享世界）**、**世界落盘（服务器重启不丢世界）**、**玩家名着色**、**远端玩家插值优化（行走动画/头部俯仰/瞬移快照）**、**观战（死亡后旁观其他玩家）**、**服务器管理面板（房间/玩家/配置/世界管理）**、**世界内换房/重建世界（`/room` `/rebuild`，无需回主菜单）**、**时间戳对齐插值（远端更平滑）**、**管理面板鉴权（可选口令）**、**手持物品外观同步（远端手部可见手持物）**、**玩家死亡掉落物（死亡散落背包、可拾取）**、**观战视角平滑**、**鉴权增强（口令过期 + 操作日志）**、**插值延迟自适应（按网络抖动自动调节）**。联机模式**不写本地存档**。
>
