...
> 阶段 3：方块共建/破坏、玩家可见与移动、互殴、死亡重生、昼夜同步、聊天、**掉落物与拾取同步**、**断线自动重连**、**怪物同步（事件同步：host 生成 + 受击/死亡广播 + 位置校正）**、**红石状态同步**、**多房间（同名房间共享世界）**、**世界落盘（服务器重启不丢世界）**、**玩家名着色**。联机模式**不写本地存档**。
>
> 联机小提示：生存模式下挖矿会掉落**物理掉落物**（谁都能拾取，掉落物 5 分钟消失）；断线后会自动重连回原房间，位置与背包不丢；夜晚会刷怪（由开房者决定生成，所有人都能打）；世界按**房间名**保存到服务器磁盘（`server/world/`），重启服务器后进同名房间即可接着玩。
...
│   ├── index.mjs        # 入口（http + WebSocket /ws，多房间管理）
│   ├── room.js          # 房间：玩家集合、方块账本、消息路由、世界恢复
