...
> 阶段 2：方块共建/破坏、玩家可见与移动、互殴、死亡重生、昼夜同步、聊天、**掉落物与拾取同步**、**断线自动重连**、**怪物同步（事件同步：host 生成 + 受击/死亡广播 + 位置校正）**、**红石状态同步**。联机模式**不写本地存档**。
>
> 联机小提示：生存模式下挖矿会掉落**物理掉落物**（谁都能拾取，掉落物 5 分钟消失）；断线后会自动重连回原房间，位置与背包不丢；夜晚会刷怪（由开房者决定生成，所有人都能打）。
...
node server/test-mp.mjs    # 跑 23 项协议断言（含方块/玩家/掉落物/怪物/红石/离开广播）
```
