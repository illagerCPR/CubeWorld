...
> 阶段 5：方块共建/破坏、玩家可见与移动、互殴、死亡重生、昼夜同步、聊天、**掉落物与拾取同步**、**断线自动重连**、**怪物同步（事件同步：host 生成 + 受击/死亡广播 + 位置校正）**、**红石状态同步**、**多房间（同名房间共享世界）**、**世界落盘（服务器重启不丢世界）**、**玩家名着色**、**远端玩家插值优化（行走动画/头部俯仰/瞬移快照）**、**观战（死亡后旁观其他玩家）**、**服务器管理面板（房间/玩家/配置/世界管理）**、**世界内换房/重建世界（`/room` `/rebuild`，无需回主菜单）**、**时间戳对齐插值（远端更平滑）**、**管理面板鉴权（可选口令）**。联机模式**不写本地存档**。
>
> 联机小提示：生存模式下挖矿会掉落**物理掉落物**（谁都能拾取，掉落物 5 分钟消失）；断线后会自动重连回原房间，位置与背包不丢；夜晚会刷怪（由开房者决定生成，所有人都能打）；世界按**房间名**保存到服务器磁盘（`server/world/`），重启服务器后进同名房间即可接着玩；死亡后点「观战其他玩家」可旁观房间内玩家（F5 切换目标，R 重生）；游戏内按 **T** 输入 `/room <房间名>` 直接换房、`/rebuild`（房主）重建当前世界。
...
- **配置管理**：掉落物过期毫秒（`dropTtlMs`）、心跳间隔毫秒（`heartbeatMs`）、每房间最大玩家数（`maxPlayersPerRoom`）、管理口令（`adminToken`），修改即生效并写入 `server/config.json`（重启保留）。
- **广播**：向所有房间发送系统公告。
- **玩家管理**：按玩家踢出（被踢客户端停止自动重连）。
- **世界管理**：清空某房间掉落物；删除某房间（踢出所有玩家 + 移除磁盘存档 = 重置该世界）。
- **鉴权（阶段 5）**：在管理面板「配置管理」设置**管理口令**（`adminToken`，留空=关闭）后，所有 `/api/*` 接口与面板都要求输入口令（浏览器自动记住）；未授权请求返回 401，接口不会回显口令明文。
...
- 管理面板**默认无鉴权**（局域网信任环境）；如需限制，可在面板「配置管理」设置**管理口令**（`adminToken`），开启后所有 API 须带口令。
