...
- 全部方块 SVG 重绘：原版配色校准 + 结构化像素画（砖缝/木板拼条/圆石砌块/年轮/矿石晶簇等），替代现有高噪声随机纹理。
- 保持注册名/textures key/SVG 管线不变，仅换生成器实现。
