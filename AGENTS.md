...
- **Idea-3 进展（体验与性能补强池）**：A-① 音频底座已交付（`src/audio/AudioEngine.js` WebAudio 程序化合成单例：惰性解锁 installUnlock/材质九类分路 blockCategory/挖掘·放置·受击·食用·弓·箭命中接线，`sound`/`volume` 进 Settings 与视频设置面板）；A-② 怪物音/脚步/BGM 待做；B Worker 化探针先行（失败止损转分帧）、C 服务器玩家档案（独立 players 目录）未开始。防回退见 `docs/agent-notes/ui-misc.md` Idea-3A 节。
