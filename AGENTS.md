...
### 光照视觉增强批次备忘（防回退）—— 水面反射/云影/后处理泛光/体积光（L1-L3 全部交付）

- **两层架构与档位**：L-A 着色器注入层（水面反射+云影，"基础"档起）+ L-B 后处理层（泛光/色调映射/体积光，"完整"档）。视频设置三档一键 `gfx: off|basic|full`（**默认 off**，老玩家画面零变化）+ 完整档子开关 `gfxBloom/gfxGodRays`。设置 key `cubeworld-settings` 全局不按存档。关闭档必须逐字节现状：`uWaterFx/uCloudShadow=0`、`GfxState.lightBoost=1`、云色不调制、`toneMapping=NoToneMapping`、postfx 关闭。
- **uniform 开关零重建（勿回退成切换设置时 markAllDirty）**：所有新效果挂在共享 uniform（uWaterFx/uCloudShadow/uTime/uSunDir/uSunColor/uSkyColor/uWind/uCloudTex/uCloudsY），着色器编译一次、切档只写 uniform。VoxelLight.js 双注入器：`applyVoxelLight`（solid+云影）与 `applyVoxelLightWater`（water+云影+菲涅尔+太阳月亮高光+方块光倒影+波纹）；水面注入点在 `#include <opaque_fragment>` 之后（雾/色调映射之前，反射高光同样受雾衰减）。**注意 replace 链不能对同一锚点 `#include <common>` 两次调用**（第二次会插在第一次结果之前导致声明顺序错误，曾真出 shader 编译错——合并为单次拼接）。
