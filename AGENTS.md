...
- **Idea-3 进展（体验与性能补强池）**：A 音频已全部交付（A-① 底座：`src/audio/AudioEngine.js` WebAudio 程序化合成单例、惰性解锁、材质九类分路、挖掘/放置/受击/食用/弓/箭命中接线，`sound`/`volume` 进 Settings；A-② 怪物语音 VOICE_PRESETS+全局限频+距离衰减、距离驱动脚步/落地/水花、计划式风声+BGM 和弦垫走 music 分轨、`music` 开关）；**B-0 探针通过 + B-① 地形生成 Worker 化已交付**（`src/workers/TerrainWorker.js`+Client，主世界专用门控、`World.requestChunk` 异步预取 + ensureChunk 同步不变 + broken 熔断回退、**worker 图必须补 BlockDefs 副作用导入否则全空气**、_disposeWorld terminate）；B-② 网格构建 Worker 化待做；C 服务器玩家档案（独立 players 目录）未开始。防回退见 `docs/agent-notes/ui-misc.md` Idea-3A/3A-②/3B 节（含 **eval 探针 Vite ?t= 实例分裂陷阱**）。
