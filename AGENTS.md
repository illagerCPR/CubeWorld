...
- **泛光定标（实测，勿凭直觉调参）**：RT 是 HalfFloat **线性亮度**（无 sRGB 编码）；实测图集最亮纹素（荧石/火把/岩浆）亮度仅 ~0.48-0.6，正午天空 color raw 值 0.66。三坑：①阈值 0.8 高到只有太阳过阈（泛光形同未开）；②强度 <0.8 时增量低于视觉差分阈（~6/255）也形同未开——定标 `threshold 0.65 / strength 0.9 / radius 0.5`；③**天空 mesh color 是 raw 值直进 RT（材质 color 无 sRGB 解码）**，正午 0.66 整片过阈把天洗白——解法 = 自定义蓝天掩除高亮提取（替换 `bloomPass.materialHighPassFilter.fragmentShader`——**属性名是 materialHighPassFilter 不是 material**），`step(c.b, max(c.r,c.g)*1.05+0.02)` 掩除 b 通道显著高于 r/g 的天空色；黄昏天空 0.62<0.65 本就不泛光。光源块 `lightMaterial.color` 完整档提亮 1.5（`GfxState.lightBoost` 门控，**关闭/基础档必须回 1.0 保持现状**，Game.update 每帧同步 chunkBuilder）。云色昼夜调制 `0.35+0.5*day` 同样完整档门控（云是无光照材质，夜里白亮+泛光=夜空发光云）。
...
- **日落白化修复（勿回退）**：`sunGlow` 日晕是 scale 160(≈43°屏)加法混合 sprite，透明度随太阳近地平线 `glowT=1-|sunY/200|/0.28` 爬到 0.9——完整档下泛光高亮提取(0.65)把整片过阈区糊成全屏白晕 + 体积光掩码(layer 1 含 sunGlow)再叠光幕，**t 0.71-0.75 白化曾真出**（归因 A/B：单关泛光即正常，双开全白）。修复 = Game.update gfx 块完整档分支对 `sky.sunGlow.material.opacity *= 0.3`（在 Sky.update 之后乘，基础/关闭档不衰减保持现状）。泛光/体积光叠加放大"本来无害的加法 sprite"是这类白化的通用模式，新增加法混合天体/粒子时同查一遍。
