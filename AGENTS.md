...
- **L4-B 太阳阴影（SunShadow.js，批次①已交付，勿回退）**：
  ...
  - shadow pass 节流：`autoUpdate=false` + 每 8 帧 `needsUpdate=true`；mapSize 2048；castShadow 恒定 true（关闭档 shadowMap.enabled=false 时 pass 不跑零成本）；
  ...
  - 批次②已交付：light-space snap（target 在垂直太阳方向的平面内按 texel 量化 + 保留沿光分量贴 depth 窗口；太阳近天顶 up×dir 退化时跳过量化）+ ready 门控改为"ready 前每帧请求 shadow pass"（start 时序里 needsUpdate 可能落在 shadowMap.enabled 置位前被跳过）；调参锚点 uShadowStrength=0.5 / normalBias=0.03 / mapSize 2048 / 每 8 帧节流。已知边界：玩家站坡上物理微动会带动 target（属真实跟随非缺陷）；跨 texel 格瞬间影子边缘整体跳 0.1 格（8 帧节流下不可见）；水面顶面影子未做（水材质无 shadow 注入）。
