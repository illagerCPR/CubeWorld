...
- **泛光定标（实测，勿凭直觉调参）**：RT 是 HalfFloat **线性亮度**（无 sRGB 编码）；实测图集最亮纹素（荧石/火把/岩浆）亮度仅 ~0.48-0.6，正午天空 color raw 值 0.66。三坑：①阈值 0.8 高到只有太阳过阈（泛光形同未开）；②强度 <0.8 时增量低于视觉差分阈（~6/255）也形同未开——定标 `threshold 0.72 / strength 0.9 / radius 0.5`（0.72 必须高于晨昏天空亮度扫描区间上限 ~0.66：晨昏天空 r/g 占优不被蓝天掩除，过阈即整片天白化，t 0.27/0.71-0.76 曾真出）；③**天空 mesh color 是 raw 值直进 RT（材质 color 无 sRGB 解码）**，正午 0.66 整片过阈把天洗白——解法 = 自定义蓝天掩除高亮提取（替换 `bloomPass.materialHighPassFilter.fragmentShader`——**属性名是 materialHighPassFilter 不是 material**），`step(c.b, max(c.r,c.g)*1.05+0.02)` 掩除 b 通道显著高于 r/g 的天空色；黄昏天空 0.62<0.65 本就不泛光。光源块 `lightMaterial.color` 完整档提亮 1.9（`GfxState.lightBoost` 门控，**关闭/基础档必须回 1.0 保持现状**，Game.update 每帧同步 chunkBuilder）。云色昼夜调制 `0.35+0.5*day` 同样完整档门控（云是无光照材质，夜里白亮+泛光=夜空发光云）。
