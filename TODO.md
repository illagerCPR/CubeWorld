...
- **状态**：进行中（2026-09-12 启动，按文末推荐顺序逐条实施）——A 弓蓄力、B 箭矢联机、E 铁傀儡、C 潜影盒均已交付（提交号见 git log）；剩余 D。P0-P3 生存链（燧石→挖矿→盔甲→经验→床→桶弓→动物→鞘翅→耕种，`6188f70`…`c285409`）已全部交付，"从出生到击败末影龙"核心缺口清零；以下为审计中标注"低优先"的选做项。
...
- **依赖链**：凋零骷髅（下界要塞生成表挂接——要塞结构已生成 ✓）→ 凋零骷髅头掉落 → 灵魂沙（下界生成器已有斑块）→ 凋灵 Boss（DragonAI 式三段血条+弹幕，BossBar 已有）→ 下界之星 → 信标（金字塔检测复用末地环 `detectEndRing` 结构检测套路 + 玩家 buff 系统，后者是 Player 状态机的新领域）。
- **召唤**：T 型灵魂沙×4 + 头×3 摆放检测（右键头部触发，与末地环激活同款交互）。
- **建议拆分**：凋零骷髅+头掉落可先做（补下界要塞战斗生态）；凋灵+信标另行立项。
