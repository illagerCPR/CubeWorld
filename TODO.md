...
- **状态**：进行中（2026-09-12 启动，按文末推荐顺序逐条实施）——A 弓蓄力已交付（`3571703`）、B 箭矢联机已交付（见 git log）；后续 E→C→D。P0-P3 生存链（燧石→挖矿→盔甲→经验→床→桶弓→动物→鞘翅→耕种，`6188f70`…`c285409`）已全部交付，"从出生到击败末影龙"核心缺口清零；以下为审计中标注"低优先"的选做项。
